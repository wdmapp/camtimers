...
!-----------------------------------------------------------------------
   ...
   public t_accel_startf
   public t_accel_stopf
   public t_accel_recordf
   public t_enablef
   public t_disablef
...
!-----------------------------------------------------------------------
   ...
   integer, parameter :: def_timing_detail_limit = 2           ! default
   ...
   integer, parameter :: init_num_accelerators = 0             ! init
   integer, private   :: num_accelerators = init_num_accelerators
                         ! current maximum number of accelerators per process
   ...
   logical, private   :: perf_add_detail = def_perf_add_detail
                         ...
                         ! do not cross detail level changes
...
!
   ! performance timer event name
   character(len=*), intent(in) :: event
   integer, intent(in)          :: accelid ! accelerator id
!
!---------------------------Input/Output arguments----------------------
!
   ! GPTL event handle
...
!
   ...
   integer  l_threadid                    ! local threadid
   character(len=2) cdetail               ! char variable for detail
!
!-----------------------------------------------------------------------
!
   ...
   l_threadid = 0
   if (( accelid .ge. 0 ) .and. ( accelid < num_accelerators )) then
      l_threadid = num_threads + accelid
   else
      l_threadid = -1
   endif

   if (l_threadid < 0) return
...
!pw   if ( present (handle) ) then
!pw      ierr = GPTLstart_handle_accel(event, l_threadid, handle)
!pw   else
!pw      ierr = GPTLstart_accel(event, l_threadid)
!pw   endif

   endif

   return
   end subroutine t_accel_startf
!
!========================================================================
!
   subroutine t_accel_stopf(event, accelid, handle)
!-----------------------------------------------------------------------
! Purpose: Stop an event timer for launching a task on an accelerator.
!          Associated timer for the accelerator is also "stopped", but
!          information on time is not recorded. This is instead updated
!          using t_accel_recordf. Uses accelid as a virtual thread id
!          for accelerator timer.
! Author: P. Worley and S. Abbott.
!-----------------------------------------------------------------------
!---------------------------Input arguments-----------------------------
!
...
!pw   else
!pw      ierr = GPTLstop_accel(event, l_threadid)
!pw   endif

   endif

   return
   end subroutine t_accel_stopf
!
!========================================================================
!
   subroutine t_accel_recordf(event, accelid, count_increment, &
                              time_increment, max_update, min_update, &
                              handle)
!-----------------------------------------------------------------------
! Purpose: Update timing information for an accelerator event: increment
!          number of times called, increment time spent in the event;
!          update max time per call and min time per call. If event does
!          not yet exist, creates it (but without any parent/child
!          information). Uses accelid as a virtual thread id.
! Author: P. Worley and S. Abbott.
!-----------------------------------------------------------------------
!---------------------------Input arguments-----------------------------
!
   ...
   character(len=*), intent(in) :: event
   integer, intent(in)          :: accelid         ! accelerator id
   integer, intent(in)          :: count_increment ! call count increment
   real(shr_kind_r8), intent(in):: time_increment  ! call duration increment
   real(shr_kind_r8), intent(in):: max_update      ! call max time update
   real(shr_kind_r8), intent(in):: min_update      ! call min time update
!
!---------------------------Input/Output arguments----------------------
!
   ! GPTL event handle
...
!
   ...
   if ((perf_add_detail) .AND. (cur_timing_detail < 100)) then
      ...
      write(cdetail,'(i2.2)') cur_timing_detail
      str_length = min(SHR_KIND_CM-3,len_trim(event))
      ierr = GPTLrecord(event(1:str_length)//'_'//cdetail, &
             l_threadid, count_increment, time_increment, max_update, min_update)

   else

      str_length = min(SHR_KIND_CM,len_trim(event))
      ierr = GPTLrecord(event(1:str_length), &
             l_threadid, count_increment, time_increment, max_update, min_update)

!pw   if ( present (handle) ) then
!pw      ierr = GPTLrecord_handle(event, &
!pw         l_threadid, count_increment, time_increment, max_update, min_update, handle)
!pw   else
!pw      ierr = GPTLrecord(event, &
!pw             l_threadid, count_increment, time_increment, max_update, min_update)
!pw   endif

   endif

   return
   end subroutine t_accel_recordf
!
!========================================================================
!
...
!
   subroutine t_initf(NLFilename, LogPrint, LogUnit, mpicom, MasterTask, &
                      MaxThreads, MaxAccelerators)
!-----------------------------------------------------------------------
! Purpose:  Set default values of runtime timing options
!           before namelists prof_inparam and papi_inparam are read,
...
!
   ...
   integer  max_gptl_threads      ! max OpenMP threads + max accelerators
                                  !  per process
!
!---------------------------Namelists ----------------------------------
!
   ...
   namelist /prof_inparam/ profile_disable, profile_barrier, &
   ...
   namelist /papi_inparam/ papi_ctr1_str, papi_ctr2_str,  &
...
!$omp end parallel
    ...
    if ( present(MaxAccelerators) ) then
       num_accelerators = MaxAccelerators
    else
       num_accelerators = init_num_accelerators
    endif
    ...
    if ( MasterTask2 ) then
       ...
       ! Read in the prof_inparam namelist from NLFilename if it exists

       write(p_logunit,*) '(t_initf) Read in prof_inparam namelist from: '//trim(NLFilename)
       unitn = shr_file_getUnit()
       ...
       if (ierr .eq. 0) then
          ...
          ! Look for prof_inparam group name in the input file.
          ! If found, leave the file positioned at that namelist group.
          call find_group_name(unitn, 'prof_inparam', status=ierr)

          if (ierr == 0) then  ! found prof_inparam
             read(unitn, nml=prof_inparam, iostat=ierr)
             if (ierr /= 0) then
                call shr_sys_abort( subname//':: namelist read returns an'// &
                                    ' error condition for prof_inparam' )
             end if
          end if
    ...
    if (perf_papi_enable) then
       ...
       if ( MasterTask2 ) then
          ...
          ! Read in the papi_inparam namelist from NLFilename if it exists

          write(p_logunit,*) '(t_initf) Read in papi_inparam namelist from: '//trim(NLFilename)
          unitn = shr_file_getUnit()

          ierr = 1
          open( unitn, file=trim(NLFilename), status="OLD", form="FORMATTED", access="SEQUENTIAL", iostat=ierr )
          if (ierr .eq. 0) then
             ! Look for papi_inparam group name in the input file.
             ! If found, leave the file positioned at that namelist group.
             call find_group_name(unitn, 'papi_inparam', status=ierr)

             if (ierr == 0) then  ! found papi_inparam
                read(unitn, nml=papi_inparam, iostat=ierr)
                if (ierr /= 0) then
                   call shr_sys_abort( subname//':: namelist read returns an'// &
                                      ' error condition for papi_inparam' )
                end if
             end if
...
!$OMP MASTER
   ...
   !
   ! Set maximum number of threads (including virtual threads to support accelerators)
   !
   if ((present(MaxThreads)) .or. (present(MaxAccelerators))) then
      max_gptl_threads = num_threads + num_accelerators
      if (gptlsetoption (gptlmaxthreads, max_gptl_threads) < 0) &
         call shr_sys_abort (subname//':: gptlsetoption')
   endif
   !
   ! Set max timer depth
